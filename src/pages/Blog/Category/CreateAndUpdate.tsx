...
import { createCategory, updateCategory } from '@/services/blog/category';
    ...
    >
      <Form
        ...
        onFinish={async (valuse) => {
          if (type == 'create') {
            setLoading(true);
            const { code } = await createCategory(valuse);
            setLoading(false);
            if (code == 200) {
              onOk(valuse);
            }
          ...
          if (type == 'update') {
            if (data?.id) {
              setLoading(true);
              const { code } = await updateCategory(valuse, data.id);
              setLoading(false);
              if (code == 200) {
                onOk(valuse);
              }
